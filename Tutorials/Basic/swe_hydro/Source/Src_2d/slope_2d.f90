...
contains
  ...
  subroutine slopex(lo, hi, &
    ...
    integer, intent(in) :: lo(2), hi(2), qlo(2), qhi(2), dqlo(2), dqhi(2), Ncomp
    double precision, intent(in ) ::  q( qlo(1): qhi(1), qlo(2): qhi(2),Ncomp)
    double precision, intent(out) :: dq(dqlo(1):dqhi(1),dqlo(2):dqhi(2),Ncomp)
    ...
    do n = 1, Ncomp
        ...
        do j    = lo(2), hi(2)
           do i = lo(1), hi(1)
              ...
              dq(i,j,n) = dsgn(i,j)*min(dlim(i,j),abs(dq1))
           end do
        end do
    end do
