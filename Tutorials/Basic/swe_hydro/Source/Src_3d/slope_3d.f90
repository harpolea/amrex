...
contains
  ...
  subroutine slopex(lo, hi, &
    ...
    integer, intent(in) :: lo(3), hi(3), qlo(3), qhi(3), dqlo(3), dqhi(3),Ncomp
    double precision, intent(in ) ::  q( qlo(1): qhi(1), qlo(2): qhi(2), qlo(3): qhi(3),Ncomp)
    double precision, intent(out) :: dq(dqlo(1):dqhi(1),dqlo(2):dqhi(2),dqlo(3):dqhi(3),Ncomp)
  ...
  subroutine slopey(lo, hi, &
    ...
    call slopey_doit(lo, hi, &
                     q, qlo, qhi, &
                     dq, dqlo, dqhi, &
                     dsgn, dlim, df, dcen, (/lo(1),lo(2)-1/), (/hi(1),hi(2)+1/),Ncomp)
  ...
  subroutine slopey_doit(lo, hi, &
                         q, qlo, qhi, &
                         dq, dqlo, dqhi, &
                         dsgn, dlim, df, dcen, ddlo, ddhi, Ncomp)
        ...
        do k = lo(3), hi(3)
           ...
           do j    = lo(2), hi(2)
              do i = lo(1), hi(1)
                 ...
                 dq(i,j,k,n) = dsgn(i,j)*min(dlim(i,j),abs(dq1))
              end do
           end do
  ...
  subroutine slopez(lo, hi, &
    ...
    call slopez_doit(lo, hi, &
                     ...
                     dq, dqlo, dqhi, &
                     dsgn, dlim, df, dcen, &
                     (/lo(1),lo(2),lo(3)-1/), (/hi(1),hi(2),hi(3)+1/),Ncomp)
        ...
        do k       = lo(3), hi(3)
           do j    = lo(2), hi(2)
              do i = lo(1), hi(1)
                 ...
                 dq(i,j,k,n) = dsgn(i,j,k)*min(dlim(i,j,k),abs(dq1))
              end do
           end do
        end do
    end do
